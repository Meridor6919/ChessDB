...
void Game::DrawGrid(DirectX::PrimitiveBatch<DirectX::VertexPositionColor> *primitive_batch)
{
	...
	DirectX::XMFLOAT3 v1(grid_rect.left,  grid_rect.top, 1.0f);
	DirectX::XMFLOAT3 v2(grid_rect.left + grid_rect.right, grid_rect.top, 1.0f);
	DirectX::XMFLOAT3 v3(grid_rect.left + grid_rect.right, grid_rect.top + grid_rect.bottom, 1.0f);
	DirectX::XMFLOAT3 v4(grid_rect.left,  grid_rect.top +  grid_rect.bottom, 1.0f);

	for (int i = 0; i <cells_in_row; ++i)
	{
		v1.x = grid_rect.left;
		v2.x = grid_rect.left + grid_rect.right;
		v3.x = grid_rect.left + grid_rect.right;
		v4.x = grid_rect.left;
		for (int j = 0; j < cells_in_column; ++j)
		{
			...
			v1.x += grid_rect.right;
			v2.x += grid_rect.right;
			v3.x += grid_rect.right;
			v4.x += grid_rect.right;
		}
		v1.y += grid_rect.bottom;
		v2.y += grid_rect.bottom;
		v3.y += grid_rect.bottom;
		v4.y += grid_rect.bottom;
		current_color = !current_color;
	}
}
